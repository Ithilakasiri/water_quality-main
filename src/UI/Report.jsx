...
const Reports = () => {
  ...
  const downloadPDF = (clientName) => {
    const doc = new jsPDF();
    doc.text(`Sensor Data Summary for ${clientName}`, 10, 10);

    // Add charts to the PDF
    const charts = [
      { id: "svmChart", title: "SVM" },
      { id: "treeChart", title: "Decision  Tree" },
      { id: "ph-histogram", title: "pH Histogram" },
      { id: "tds-histogram", title: "TDS Histogram" },
      { id: "turbidity-histogram", title: "Turbidity Histogram" },
      { id: "ph-heatmap", title: "pH Heatmap" },
      { id: "tds-heatmap", title: "TDS Heatmap" },
      { id: "turbidity-heatmap", title: "Turbidity Heatmap" },
    ];

    let y = 20; // Initial y position after the table
    const spaceBetweenCharts = 20; // Space between charts
    ...
    doc.autoTable({
        ...
        body: statistics.map(({ key, stats }) => [
          key,
          stats ? stats.count : "No Data",
          stats ? stats.mean : "No Data",
          stats ? stats.std : "No Data",
          stats ? stats.min : "No Data",
          stats ? stats.q25 : "No Data",
          stats ? stats.median : "No Data",
          stats ? stats.q75 : "No Data",
          stats ? stats.max : "No Data",
        ]),
      });

    doc.save("sensor_data_summary.pdf");
  };
    ...
    <Container className="mt-4">
      ...
      {visualizeClicked && (
        <>
          ...
          <Row>
            <Button onClick={() => downloadPDF(clientName)} variant="secondary" className="w-100">
              Download PDF
            </Button>
          </Row>
        </>
